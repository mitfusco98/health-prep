...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not user.is_admin:
            logger.warning(
                f"Non-admin user {user.username} attempted to access admin route: {request.endpoint}"
            )
        ...
        # Log admin access
        logger.info(
            f"Admin access granted to user {user.username} for route: {request.endpoint}"
        )
...
def register_admin_middleware(app):
    ...
    def protect_admin_routes():
        ...
        if request.path.startswith("/admin"):
            ...
            if not user.is_admin:
                logger.warning(
                    f"Non-admin user {user.username} attempted to access admin route: {request.path}"
                )
                flash("Access denied. Administrator privileges required.", "error")
                return redirect(url_for("index"))
            ...
            # Log admin access
            logger.info(
                f"Admin middleware: Access granted to user {user.username} for {request.path}"
            )
