...
class AppProfiler:
            ...
            try:
                ...
                with self.lock:
                    ...
                    if len(self.route_stats[route_name]) > 100:
                        self.route_stats[route_name] = self.route_stats[route_name][
                            -100:
                        ]
                ...
                if duration > 500:
                    logger.warning(
                        f"Slow route detected: {route_name} took {duration:.2f}ms"
                    )
    ...
    def get_slowest_routes(self, limit=10):
        ...
        return sorted(route_averages, key=lambda x: x["avg_duration_ms"], reverse=True)[
            :limit
        ]
    ...
    def get_slowest_functions(self, limit=10):
        ...
        return sorted(
            function_averages, key=lambda x: x["avg_duration_ms"], reverse=True
        )[:limit]
