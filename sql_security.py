    ...
    def validate_input(cls, value):
        ...
        for pattern in cls.DANGEROUS_PATTERNS:
            if re.search(pattern, value_str, re.IGNORECASE | re.DOTALL):
                logger.warning(
                    f"Dangerous SQL pattern detected: {pattern} in input: {value_str[:100]}"
                )
                return False
    ...
    if parameters:
        for key, value in parameters.items():
            if not SQLSecurityValidator.validate_input(value):
                raise ValueError(
                    f"Potentially dangerous SQL patterns detected in parameter {key}"
                )
...
def get_patient_by_mrn_safe(mrn):
    ...
    try:
        result = execute_safe_query(
            "SELECT * FROM patient WHERE mrn = :mrn LIMIT 1", {"mrn": mrn}
        )
...
def search_patients_safe(search_term, limit=50):
    ...
    try:
        ...
        result = execute_safe_query(
            ...
            LIMIT :limit
        """,
            {"search_pattern": f"%{escaped_term}%", "limit": limit},
        )
