...
def ocr_reprocess_low_confidence():
    ...
    try:
        ...
        flash(f"Reprocessed {len(document_ids)} low confidence documents. Success: {successful_ocr}, Failed: {failed_ocr}", "info")
        return redirect(url_for('ocr_dashboard'))

    except Exception as e:
        logger.error(f"Error in bulk low confidence reprocessing: {e}")
        flash(f"Error reprocessing documents: {str(e)}", "error")
        return redirect(url_for('ocr_dashboard'))


@app.route('/admin/ocr-bulk-process', methods=['POST'])
def ocr_bulk_process():
    """Bulk process multiple documents with OCR"""
    try:
        ...
        if not document_ids:
            return jsonify({
                'success': False,
                'error': 'No documents selected for processing'
            })
        ...
        return jsonify({
            ...
            'message': f'Processed {len(document_ids)} documents. Success: {successful_ocr}, Failed: {failed_ocr}'
        })

    except Exception as e:
        logger.error(f"Error in bulk OCR processing: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        })
...
@app.route('/admin/ocr-process-document/<int:doc_id>', methods=['POST'])
@csrf.exempt
...
def ocr_process_pending():
    ...
    try:
        ...
        flash(f"Processed {len(pending_doc_ids)} pending documents. Success: {successful_ocr}, Failed: {failed_ocr}", "success")
        return redirect(url_for('ocr_dashboard'))

    except Exception as e:
        logger.error(f"Error processing pending OCR documents: {e}")
