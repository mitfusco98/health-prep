...
class ValidationLogger:
    ...
    @staticmethod
    def log_validation_failure(endpoint, validation_errors, form_data=None, user_id=None):
        """
        ...
        try:
            ...
            safe_form_data = ValidationLogger._sanitize_form_data(form_data) if form_data else {}

            # Format validation errors
            if isinstance(validation_errors, dict):
                error_summary = "; ".join([f"{field}: {', '.join(errors)}" for field, errors in validation_errors.items()])
            ...
            details = {
                ...
                'error_count': len(validation_errors) if isinstance(validation_errors, (dict, list)) else 1
            }
    ...
    def _sanitize_form_data(form_data):
        ...
        sensitive_fields = {
            ...
            'new_password', 'old_password', 'csrf_token', 'api_key', 'secret',
            'token', 'auth_token', 'session_token', 'ssn', 'social_security'
        }
...
def register_validation_middleware(app):
    ...
    def handle_validation_error(error):
        ...
        return error.get_response() if hasattr(error, 'get_response') else str(error), 400
