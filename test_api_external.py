...
class APITester:
        ...
        try:
            ...
            if response.status_code == expected_status:
                ...
                try:
                    ...
                    self.log(
                        f"Response data: {json.dumps(json_data, indent=2)[:200]}..."
                    )
                except:
    ...
    def quick_health_check(self):
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/auth/login", json={}, timeout=5
            )
    ...
    def test_auth_flow(self):
        ...
        response = self.session.post(
            f"{self.base_url}/api/auth/register", json=TEST_USER
        )
        if response.status_code == 201:
            ...
            self.log(
                "✅ PASS - User already exists (expected for repeated tests)", "SUCCESS"
            )
            self.passed_tests += 1
        else:
            self.log(
                f"❌ FAIL - Expected 201 or 409, got {response.status_code}", "ERROR"
            )
            self.failed_tests += 1
        ...
        # 8. Test logout
        self.test_endpoint(
            "POST", "/api/auth/logout", expected_status=200, description="User logout"
        )
    ...
    def test_patients_api(self):
        ...
        self.test_endpoint(
            "GET", "/api/patients", expected_status=200, description="Get patients list"
        )
    ...
    def test_security_vulnerabilities(self):
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/api/patients", json={"test": "data"}, headers=headers
            )
            if response.status_code in [401, 403]:
                ...
                self.log(
                    "⚠️  CSRF protection may be missing (this is expected for test environment)"
                )
    ...
    def test_error_handling(self):
        ...
        try:
            ...
            if response.status_code == 400:
                ...
                self.log(
                    f"❌ FAIL - Expected 400 for malformed JSON, got {response.status_code}",
                    "ERROR",
                )
                self.failed_tests += 1
        except Exception as e:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/api/auth/login", json=large_payload, timeout=5
            )
    ...
    def test_rate_limiting(self):
        ...
        for i in range(20):
            try:
                ...
                if response.status_code == 429:
                    self.log(
                        f"✅ PASS - Rate limiting active after {requests_made} requests",
                        "SUCCESS",
                    )
                    self.passed_tests += 1
                    return
        ...
        self.log(
            f"⚠️  WARNING - No rate limiting detected after {requests_made} rapid requests",
            "WARNING",
        )
        self.failed_tests += 1
    ...
    def test_performance(self):
        ...
        endpoints = ["/api/patients", "/api/appointments", "/api/auth/verify"]
...
def main():
    ...
    if args.quick:
        ...
        success_rate = (
            (tester.passed_tests / total_tests * 100) if total_tests > 0 else 0
        )
