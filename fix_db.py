...
try:
    ...
    if DB_URL:
        ...
        with conn.cursor() as cursor:
            ...
            print(
                "Checking if document_name column exists in medical_document table..."
            )
            cursor.execute(
                "SELECT column_name FROM information_schema.columns WHERE table_name = 'medical_document' AND column_name = 'document_name'"
            )
            if not cursor.fetchone():
                print("document_name column doesn't exist. Adding it now...")
                cursor.execute(
                    "ALTER TABLE medical_document ADD COLUMN document_name VARCHAR(255)"
                )
                ...
                cursor.execute(
                    "UPDATE medical_document SET document_name = filename WHERE document_name IS NULL AND filename IS NOT NULL"
                )
                print(
                    f"Updated {cursor.rowcount} records with document names from filenames."
                )
            else:
