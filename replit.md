...
## Changelog
- July 16, 2025: **UNIFIED VARIANT STATUS MANAGEMENT & A1C KEYWORD FIX**: Implemented comprehensive unified status activity system for screening type variants. When one variant is deactivated/activated, all related variants automatically sync to the same status, ensuring unified behavior across /screenings?tab=types, /screenings?tab=checklist, and /screenings?tab=screenings tabs. Enhanced variant manager with automatic cascade cleanup and proper cross-variant synchronization. **FIXED A1C FALSE MATCHING**: Resolved critical issue where A1C screenings incorrectly matched thyroid panel documents. Updated A1C keywords from overly broad ["a1c", "sugar", "glucose"] to specific medical terms ["hemoglobin a1c", "hba1c", "glycated hemoglobin", "diabetes control", "blood sugar control"]. Cleaned up 3 incorrect A1C-thyroid document relationships and reset affected screening statuses. System now prevents false positive matches while maintaining medical accuracy.
