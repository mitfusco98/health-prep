...
## Changelog
- July 16, 2025: **UNIFIED VARIANT STATUS MANAGEMENT & KEYWORD MATCHING FIX**: Implemented comprehensive unified status management for screening type variants. A1C variants and other screening variants now share unified active/inactive status on /screenings?tab=types. When one variant is deactivated, all related variants are automatically deactivated through the sync_variant_statuses system. Fixed template to display consolidated screening entries showing base names with variant counts and unified status buttons. **CRITICAL KEYWORD MATCHING FIX**: Resolved A1C screenings incorrectly matching thyroid panel documents by replacing overly broad keywords ("sugar", "glucose") with specific medical terms ("hemoglobin a1c", "hba1c", "glycosylated hemoglobin"). Fixed SQLAlchemy relationship clearing bug in automated edge case handler that was causing 'AppenderQuery' object errors during screening cleanup. System now properly manages document-screening relationships and prevents false positive matches.
