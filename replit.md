...
## Changelog
- July 17, 2025: **CRITICAL RELIABILITY FIX - UNIFIED SCREENING ENGINE**: Completely rebuilt the screening generation system to eliminate all reliability issues and inconsistent keyword matching. **Root Problems Identified**: Multiple competing keyword matching systems creating contradictory results, problematic fallback to screening type names causing false positives, scattered demographic filtering logic, AttributeError in condition checking (`condition_name` vs `name`), duplicate screening types in database. **Comprehensive Solution**: Created unified_screening_engine.py as single authoritative source for all screening operations; Fixed condition attribute error (c.name not c.condition_name); Consolidated duplicate HbA1c Test and Vaccination History entries; Updated automated_screening_engine.py to use unified engine exclusively; Removed unreliable fallback matching that used screening names; Added proper FK relationships (screening_type_id) and data preservation columns (is_visible, is_system_generated) to Screening model. **Verified Results**: All 60 screenings confirmed present for eligible patients (12 patients, 8 active screening types); Query optimization shows proper filtering by ScreeningType.is_active and Screening.is_visible; Status distribution shows appropriate Complete/Due/Incomplete classifications; No missing eligible patients - coverage gaps = 0 across all screening types. System now operates with single, reliable engine eliminating inconsistency issues.
