...
## Changelog
- July 17, 2025: **COMPREHENSIVE VARIANT MANAGEMENT & CONDITION MATCHING FIX**: Fixed unified variant status management system to properly detect and synchronize new variant cases. Enhanced `edit_screening_type` and `delete_screening_type` routes to use `ScreeningVariantManager.sync_single_variant_status()` method ensuring all related variants (including exact duplicate names) sync their active/inactive status together. **TESTED & VERIFIED**: Vaccination History variants (IDs 22, 26) with different age ranges now properly function as unified screening type - activating one automatically activates all related variants. Fixed condition-based trigger matching with robust variant detection system supporting diabetes/hypertension/hyperlipidemia/obesity condition families and ICD-10 code relationships. System now correctly matches "Type 2 Diabetes Mellitus" (E11.9) with "Diabetes mellitus" trigger conditions (73211009). Enhanced document categorization to support both legacy (RADIOLOGY_REPORT) and new form types (IMAGING, LABORATORIES, CONSULTS, HOSPITAL_RECORDS). Olivia Garcia now properly eligible for diabetes-related screenings.
