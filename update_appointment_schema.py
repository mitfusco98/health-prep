...
def add_status_column():
    ...
    try:
        with app.app_context():
            ...
            result = conn.execute(text("SELECT column_name FROM information_schema.columns WHERE table_name='appointment' AND column_name='status'"))
            columns = result.fetchall()
            
            if not columns:
                logger.info("Adding 'status' column to appointment table...")
                conn.execute(text("ALTER TABLE appointment ADD COLUMN status VARCHAR(20) DEFAULT 'waiting'"))
                conn.commit()
