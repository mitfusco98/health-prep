...
def update_checklist_settings():
    ...
    settings.status_options = ','.join(status_options) if status_options else 'due,due_soon'
    settings.custom_status_options = ','.join(custom_status_options) if custom_status_options else ''
...
def update_checklist_generation():
    ...
    # Update settings
    settings.content_sources = ','.join(content_sources) if content_sources else 'database'
...
def remove_custom_status():
    ...
    if status_to_remove in custom_statuses:
        ...
        settings.custom_status_options = ','.join(custom_statuses) if custom_statuses else ''
        db.session.commit()
