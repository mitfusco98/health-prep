...
class CSVProcessor:
    ...
    def process(self, csv_file):
        ...
        try:
            ...
            if csv_type not in self.processors:
                return {
                    "success": False,
                    "error": f"Unsupported CSV format: {csv_type}",
                }
    ...
    def _validate_basic_headers(self, headers):
        ...
        has_data = any(
            header in headers
            for header in ["first_name", "condition_name", "test_name", "visit_date"]
        )
    ...
    def _determine_csv_type(self, headers):
        ...
        if "mrn" in headers and "first_name" in headers and "last_name" in headers:
            ...
            "patient_mrn" in headers
            and "test_name" in headers
            and "test_date" in headers
        ):
            return "lab_results"
    ...
    def _create_condition_for_patient(self, row):
        ...
        existing_condition = Condition.query.filter_by(
            patient_id=patient.id, name=row["condition_name"]
        ).first()
...
def evaluate_screening_needs(patient):
    ...
    for rec in screening_recommendations:
        ...
        existing = Screening.query.filter_by(
            patient_id=patient.id, screening_type=rec["type"]
        ).first()
...
class PatientPrepData:
    ...
    def from_patient_id(cls, patient_id):
        ...
        prep_data.recent_vitals = (
            Vital.query.filter(
                Vital.patient_id == patient_id, Vital.date >= six_months_ago
            )
        ...
        prep_data.active_conditions = Condition.query.filter(
            Condition.patient_id == patient_id, Condition.is_active == True
        ).all()
...
class PrepSheetGenerator:
    ...
    def _generate_summary(self, prep_data):
        ...
        if len(prep_data.active_conditions) > 3:
            condition_str += (
                f", and {len(prep_data.active_conditions) - 3} other conditions"
            )
    ...
    def _generate_hospital_alerts(self, prep_data):
        ...
        for stay in prep_data.recent_hospital:
            days_hospitalized = (
                (stay.discharge_date - stay.admission_date).days
...
def generate_prep_sheet(
    ...
    recent_labs=None,
    recent_imaging=None,
    recent_consults=None,
    recent_hospital=None,
    ...
    prep_data.recent_labs = (
        (recent_labs or [])[:5] if not include_full_data else (recent_labs or [])
    )
    prep_data.recent_imaging = (
        (recent_imaging or [])[:3] if not include_full_data else (recent_imaging or [])
    )
    prep_data.recent_consults = (
        (recent_consults or [])[:3]
        if not include_full_data
        else (recent_consults or [])
    )
    prep_data.recent_hospital = (
        (recent_hospital or [])[:2]
        if not include_full_data
        else (recent_hospital or [])
    )
    prep_data.active_conditions = (
        (active_conditions or [])[:10]
        if not include_full_data
        else (active_conditions or [])
    )
    prep_data.screenings = (
        (screenings or [])[:10] if not include_full_data else (screenings or [])
    )
    prep_data.last_visit_date = last_visit_date
    prep_data.past_appointments = (
        (past_appointments or [])[:5]
        if not include_full_data
        else (past_appointments or [])
    )
    ...
    if condition_str:
        prep_sheet["summary"] = (
            f"{patient.full_name} is a {age_str} {patient.sex.lower()} with {condition_str}."
        )
    else:
        prep_sheet["summary"] = (
            f"{patient.full_name} is a {age_str} {patient.sex.lower()} with no documented active conditions."
        )
    ...
    if last_visit_date:
        ...
        prep_sheet[
            "summary"
        ] += f" Last visit was {days_since} days ago on {last_visit_date.strftime('%Y-%m-%d')}."
...
def process_document_upload(content, filename, source_system="HealthPrep"):
    ...
    result = {"success": False, "document_type": None, "metadata": {}, "error": None}
...
class DocumentClassifier:
    ...
    def _classify_discharge_summary(self, content_lower, metadata):
        if re.search(r"discharge\s+summary", content_lower) or re.search(
            r"discharged\s+from", content_lower
        ):
...
def process_document(document_id):
    ...
    provider_match = re.search(
        r"provider|physician|doctor|attending|surgeon:\s*([A-Za-z\s\.,]+)",
        content.lower(),
    )
    if provider_match:
    ...
    if doc_type == DocumentType.DISCHARGE_SUMMARY.value:
        ...
        if diag_match:
            ...
            diagnoses = [
                d.strip() for d in re.split(r"[,;]", diagnoses_text) if d.strip()
            ]
        ...
        for match in re.finditer(result_pattern, content):
            ...
            results.append({"test": test_name, "value": value, "unit": unit})
...
def get_patient_documents_summary(patient_id):
    ...
    for doc in documents:
        ...
        doc_info = {
            ...
            "document_name": (
                doc.document_name if hasattr(doc, "document_name") else doc.filename
            ),
...
def process_document_advanced(document_id):
    ...
    try:
        ...
        try:
            metadata = (
                json.loads(document.doc_metadata) if document.doc_metadata else {}
            )
        except json.JSONDecodeError:
            metadata = {}
...
def extract_imaging_data(content):
    ...
    data = {"has_impression": False, "study_type": "Unknown"}
...
def extract_consult_data(content):
    ...
    data = {"has_recommendations": False, "specialty": "Unknown"}
...
def extract_hospital_data(content):
    ...
    data = {"hospital": "Unknown", "has_discharge_instructions": False}

    # Check for discharge instructions
    if any(
        x in content.upper()
        for x in ["DISCHARGE INSTRUCTION", "FOLLOW-UP", "FOLLOW UP"]
    ):
    ...
    if "HOSPITAL" in content_upper:
        ...
        for line in lines:
            if (
                "HOSPITAL" in line.upper() and len(line) < 100
...
def get_next_available_mrn():
    ...
    try:
        ...
        result = db.session.execute(
            text(
                "SELECT MAX(CAST(SUBSTRING(mrn FROM :pattern) AS INTEGER)) FROM patient WHERE mrn ~ :regex_pattern"
            ),
