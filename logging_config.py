...
def configure_logging(app):
    ...
    if is_production:
        ...
        app.logger.info(
            "Production logging configuration loaded",
            extra={
        ...
        app.logger.info(
            "Development logging configuration loaded",
            extra={
                "event_type": "system_startup",
...
def log_application_startup(app, structured_logger):
    ...
    startup_info = {
        ...
        "database_url": (
            ...
            if "@" in app.config.get("SQLALCHEMY_DATABASE_URI", "")
            else "sqlite"
        ),
        ...
        "session_lifetime": str(
            app.config.get("PERMANENT_SESSION_LIFETIME", "default")
        ),
    }

    structured_logger.logger.info(
        "Healthcare management application started", extra=startup_info
    )
