...
from wtforms import StringField, PasswordField, SubmitField, SelectField, DateField, TextAreaField, FloatField, IntegerField, BooleanField, HiddenField
...
class PatientForm(FlaskForm):
    ...
    mrn = StringField('Medical Record Number', validators=[Optional()], description="Leave blank to auto-generate")
...
class ConditionForm(FlaskForm):
    name = StringField('Condition Name', validators=[DataRequired(), Length(min=1, max=200)])
    diagnosed_date = StringField('Date Diagnosed', validators=[DataRequired()], 
    ...
    def validate_diagnosed_date(self, field):
        ...
        raise ValidationError('Enter either a full date (YYYY-MM-DD) or just a year (YYYY)')
...
class VitalForm(FlaskForm):
    ...
    weight = FloatField('Weight (lbs)', validators=[Optional(), NumberRange(min=0, message="Weight must be a positive number")])
    height = FloatField('Height (in)', validators=[Optional(), NumberRange(min=0, message="Height must be a positive number")])
    temperature = FloatField('Temperature (°F)', validators=[Optional(), NumberRange(min=0, message="Temperature must be a positive number")])
    blood_pressure_systolic = IntegerField('Systolic BP', validators=[Optional(), NumberRange(min=1, message="Blood pressure must be a positive number")])
    blood_pressure_diastolic = IntegerField('Diastolic BP', validators=[Optional(), NumberRange(min=1, message="Blood pressure must be a positive number")])
    pulse = IntegerField('Pulse (bpm)', validators=[Optional(), NumberRange(min=1, message="Pulse must be a positive number")])
    respiratory_rate = IntegerField('Respiratory Rate', validators=[Optional(), NumberRange(min=1, message="Respiratory rate must be a positive number")])
    oxygen_saturation = FloatField('O2 Saturation (%)', validators=[Optional(), NumberRange(min=0, max=100, message="Oxygen saturation must be between 0-100%")])
...
class HospitalSummaryForm(FlaskForm):
    admission_date = DateField('Admission Date', validators=[DataRequired()], format='%Y-%m-%d')
    discharge_date = DateField('Discharge Date', validators=[Optional()], format='%Y-%m-%d')
    hospital_name = StringField('Hospital Name', validators=[DataRequired()])
    admitting_diagnosis = StringField('Admitting Diagnosis', validators=[DataRequired()])
    ...
    discharge_medications = TextAreaField('Discharge Medications', validators=[Optional()])
    followup_instructions = TextAreaField('Follow-up Instructions', validators=[Optional()])
...
class ScreeningForm(FlaskForm):
    ...
    last_completed = DateField('Last Completed', validators=[Optional()], format='%Y-%m-%d')
...
class ScreeningTypeForm(FlaskForm):
    ...
    default_frequency = StringField('Default Frequency', validators=[Optional()],
                                   description="How often this screening should be done (e.g., 'Annual', 'Every 5 years')")
    gender_specific = SelectField('Gender Specific', 
...
class ImmunizationForm(FlaskForm):
    ...
    administration_date = DateField('Administration Date', validators=[DataRequired()], format='%Y-%m-%d')
...
class DocumentUploadForm(FlaskForm):
    file = FileField('Document File (Optional)', validators=[
        Optional(),
        FileAllowed(['txt', 'pdf', 'jpg', 'jpeg', 'png'], 'Only text, PDF and common image formats allowed'),
        lambda form, field: len(field.data.read()) <= 10 * 1024 * 1024 if field.data else True  # 10MB limit
    ])
    document_name = StringField('File Name', validators=[DataRequired()],
                             description="Give this document a descriptive name")
    source_system = StringField('Source System/Origin', validators=[DataRequired()], 
                              default='HealthPrep',
...
class AppointmentForm(FlaskForm):
    ...
    appointment_time = SelectField('Time', validators=[DataRequired()],
                                 description="Available 15-minute time slots between 8:00 AM and 4:00 PM")
    note = StringField('Appointment Note', validators=[Optional()], 
...
class PatientAlertForm(FlaskForm):
    alert_type = SelectField('Alert Type', validators=[DataRequired()],
                       choices=[
                           ('Allergy', 'Allergy'),
    ...
    description = StringField('Alert Description', validators=[DataRequired(), Length(max=200)],
                       description="Brief description of the alert")
    details = TextAreaField('Alert Details', validators=[Optional()],
