...
with app.app_context():
    ...
    # Skip admin log cleanup during startup for faster boot
...
# Add basic routes to prevent import issues
@app.route('/')
@app.route('/home')
...
def index(date=None):
    ...
    try:
        ...
        ).order_by(Appointment.appointment_time).limit(20).all()
        recent_patients = Patient.query.order_by(desc(Patient.created_at)).limit(10).all()
        recent_documents = MedicalDocument.query.order_by(desc(MedicalDocument.created_at)).limit(5).all()
...
# Add essential routes to prevent template errors
@app.route('/patients')
@login_required
def patient_list():
    """Patient list page"""
    try:
        ...
        patients = Patient.query.order_by(Patient.last_name, Patient.first_name).limit(50).all()
    except Exception as e:
        patients = []
...
def appointment_list():
    ...
    try:
        ...
        appointments = Appointment.query.order_by(Appointment.appointment_date).limit(50).all()
    except Exception as e:
        appointments = []
        print(f"Appointment query failed: {e}")
