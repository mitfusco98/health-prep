...
# =============================================================================
# PREP CHECKLIST MODELS - Dynamic screening checklist system
# =============================================================================
...
class PrepChecklistItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    template_id = db.Column(db.Integer, db.ForeignKey('prep_checklist_templates.id'), nullable=False)
    
    # Basic item configuration
    section = db.Column(db.Enum(PrepChecklistSection), nullable=False)
    item_type = db.Column(db.Enum(PrepChecklistItemType), nullable=False)
    ...
    screening_type_id = db.Column(db.Integer, db.ForeignKey('screening_type.id'), nullable=True)
    
    # Keyword matching configuration
    primary_keywords = db.Column(db.Text)  # JSON array of primary keywords
    secondary_keywords = db.Column(db.Text)  # JSON array of secondary keywords
    excluded_keywords = db.Column(db.Text)  # JSON array of keywords to exclude
    
    # Age and gender rules
    min_age = db.Column(db.Integer)
    max_age = db.Column(db.Integer)
    gender_specific = db.Column(db.String(20))  # 'Male', 'Female', or None for all
    
    # Condition triggers
    trigger_conditions = db.Column(db.Text)  # JSON array of condition names/codes
    
    # Matching configuration
    match_confidence_threshold = db.Column(db.Integer, default=70)  # 0-100
    require_recent_data = db.Column(db.Boolean, default=True)
    recent_data_days = db.Column(db.Integer, default=365)
    
    # Display configuration
    show_in_summary = db.Column(db.Boolean, default=True)
    ...
    # Relationships
    template = db.relationship("PrepChecklistTemplate", backref="checklist_items")
    screening_type = db.relationship("ScreeningType", foreign_keys=[screening_type_id])
...
class PrepChecklistResult(db.Model):
    ...
    patient_id = db.Column(db.Integer, db.ForeignKey('patient.id'), nullable=False)
    template_id = db.Column(db.Integer, db.ForeignKey('prep_checklist_templates.id'), nullable=False)
    checklist_item_id = db.Column(db.Integer, db.ForeignKey('prep_checklist_items.id'), nullable=False)
    
    # Evaluation results
    match_status = db.Column(db.Enum(PrepChecklistMatchStatus), default=PrepChecklistMatchStatus.NOT_CHECKED)
    confidence_score = db.Column(db.Integer, default=0)  # 0-100
    
    # Matched data details
    matched_data_type = db.Column(db.String(50))  # 'lab', 'imaging', 'consult', 'hospital', 'screening'
    matched_data_id = db.Column(db.Integer)  # ID of the matched record
    matched_keywords = db.Column(db.Text)  # JSON array of keywords that matched
    matched_text_snippet = db.Column(db.Text)  # Excerpt from matched document/data
    
    # Manual overrides
    manual_status = db.Column(db.String(20))  # Manual status set by user
    ...
    data_date = db.Column(db.DateTime)  # Date of the matched data
    ...
    template = db.relationship("PrepChecklistTemplate")
    checklist_item = db.relationship("PrepChecklistItem")
    ...
    def __repr__(self):
        return f"<PrepChecklistResult {self.checklist_item.title if self.checklist_item else 'Unknown'} - {self.match_status.value}>"
...
class PrepChecklistConfiguration(db.Model):
    ...
    # Default template
    default_template_id = db.Column(db.Integer, db.ForeignKey('prep_checklist_templates.id'))
    
    # Evaluation settings
    auto_evaluate_on_data_add = db.Column(db.Boolean, default=True)
    require_manual_review = db.Column(db.Boolean, default=False)
    confidence_threshold = db.Column(db.Integer, default=70)
    
    # Data freshness settings
    max_lab_age_days = db.Column(db.Integer, default=365)
    max_imaging_age_days = db.Column(db.Integer, default=730)
    max_consult_age_days = db.Column(db.Integer, default=365)
    max_hospital_age_days = db.Column(db.Integer, default=1095)
    
    # Display settings
    show_confidence_scores = db.Column(db.Boolean, default=True)
    group_by_section = db.Column(db.Boolean, default=True)
    highlight_missing_items = db.Column(db.Boolean, default=True)
    
    # Integration settings
    sync_with_screening_types = db.Column(db.Boolean, default=True)
    auto_create_screening_items = db.Column(db.Boolean, default=True)
    
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationships
    default_template = db.relationship("PrepChecklistTemplate", foreign_keys=[default_template_id])
