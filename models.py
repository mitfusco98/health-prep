...
class User(UserMixin, db.Model):
    ...
    def set_password(self, password):
        ...
        self.password_hash = generate_password_hash(password, method='pbkdf2:sha256:600000', salt_length=32)
...
class Patient(db.Model):
    ...
    hospital_summaries = db.relationship('HospitalSummary', backref='patient', lazy=True)
...
class Visit(db.Model):
    ...
    visit_type = db.Column(db.String(50))  # e.g., 'Annual Physical', 'Follow-up', 'Urgent'
...
class HospitalSummary(db.Model):
    ...
    def __repr__(self):
        return f'<HospitalSummary for Patient {self.patient_id} on {self.admission_date}>'
...
class Screening(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    patient_id = db.Column(db.Integer, db.ForeignKey('patient.id'), nullable=False)
    screening_type = db.Column(db.String(100), nullable=False)  # e.g., 'Mammogram', 'Colonoscopy'
...
class MedicalDocument(db.Model):
    ...
    document_type = db.Column(db.String(50))  # Stores the string value of DocumentType enum
    content = db.Column(db.Text, nullable=True)  # Text content (nullable to allow binary-only files)
    binary_content = db.Column(db.LargeBinary, nullable=True)  # Binary content for images
...
class EHRConnection(db.Model):
    ...
    # Relationships
    import_history = db.relationship('EHRImportHistory', backref='connection', lazy=True)
...
class EHRImportHistory(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    connection_id = db.Column(db.Integer, db.ForeignKey('ehr_connection.id'), nullable=False)
    ...
    imported_data_types = db.Column(db.String(255))  # Comma-separated list: 'patient,conditions,vitals,documents'
...
class Appointment(db.Model):
    ...
    status = db.Column(db.String(20), default="OOO")  # Options: 'OOO', 'waiting', 'provider', 'seen'
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class PatientAlert(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    patient_id = db.Column(db.Integer, db.ForeignKey('patient.id'), nullable=False)
    alert_type = db.Column(db.String(50), nullable=False)  # e.g., 'Allergy', 'Clinical', 'Administrative'
...
class ChecklistSettings(db.Model):
    ...
    status_options = db.Column(db.Text, default='due,due_soon,sent_incomplete,completed')  # Comma-separated list
    custom_status_options = db.Column(db.Text, nullable=True)  # Comma-separated list of custom status options
    content_sources = db.Column(db.Text, default='database,age_based,gender_based,condition_based')  # Comma-separated list
    default_items = db.Column(db.Text, nullable=True)  # Newline-separated list of default items
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class AdminLog(db.Model):
    ...
    event_type = db.Column(db.String(50), nullable=False, index=True)  # login_fail, validation_error, admin_action, etc.
    event_details = db.Column(db.Text, nullable=True)  # JSON string with additional details
    request_id = db.Column(db.String(36), nullable=True, index=True)  # UUID for request tracking
