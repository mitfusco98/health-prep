...
def fix_admin_logs():
    ...
    try:
        with app.app_context():
            ...
            for log in logs:
                ...
                if log.user_id and not log.user:
                    try:
                        ...
                        logger.error(f"Error fixing user info for log {log.id}: {str(e)}")
                    ...
                    try:
                        ...
                        if details and 'parsed' in details and not details['parsed']:
                            ...
                            if 'route' in raw_data:
                                import re
                                route_match = re.search(r"'route':\s*'([^']*)'", raw_data)
                                if route_match:
                                    new_details['route'] = route_match.group(1)
                                    
                            if 'method' in raw_data:
                                method_match = re.search(r"'method':\s*'([^']*)'", raw_data)
                                if method_match:
                        ...
                        logger.error(f"Error fixing event details for log {log.id}: {str(e)}")
            ...
            logs_with_users = AdminLog.query.filter(AdminLog.user_id.isnot(None)).count()
            logs_with_details = AdminLog.query.filter(AdminLog.event_details.isnot(None)).count()
