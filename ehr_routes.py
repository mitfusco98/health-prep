...
from ehr_integration import ehr_service, fhir_service, EHRConnectionConfig, EHRVendor
...
def ehr_integration():
    ...
    # Get import history
    import_history = (
        EHRImportHistory.query.order_by(EHRImportHistory.import_date.desc())
        .limit(10)
        .all()
    )

    return render_template(
        "ehr_integration.html",
...
def add_ehr_connection():
    ...
    try:
        ...
        connection = EHRConnection(
            name=name, vendor=vendor, base_url=base_url, auth_type=auth_type
        )
        ...
        if auth_type == "api_key":
            ...
            if api_key and validate_api_key_security(api_key):
                ...
                flash(
                    "Invalid API key format or service-level key detected. Use public/anon keys only.",
                    "danger",
                )
                return redirect(url_for("ehr_integration"))
            ...
            if (
                client_id
                and client_secret
                and validate_oauth_credentials(client_id, client_secret)
            ):
...
def test_ehr_connection():
    ...
    try:
        ...
        if result:
            ...
            flash(
                f"Connection test failed. Please check your configuration and try again.",
                "danger",
            )
...
def import_ehr_patient():
    ...
    try:
        ...
        fhir_patient_data = fhir_service.get_patient(
            connection_name=connection.name, patient_id=patient_id
        )

        if not fhir_patient_data:
            import_history.success = False
            import_history.error_message = (
                f"Patient with ID {patient_id} not found in EHR system."
            )
        ...
        if import_patient_data:
            patient = fhir_service.import_patient(
                connection_name=connection.name, fhir_patient=fhir_patient_data
            )
        ...
        if patient:
            flash(
                f"Successfully imported {imported_items} items for {patient.full_name}.",
                "success",
            )
    ...
    with app.app_context():
        ...
        for connection in connections:
            try:
                ...
                logger.error(
                    f"Error importing EHR connection {connection.name}: {str(e)}"
                )
