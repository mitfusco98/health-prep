...
from models import Patient, Condition, Screening, Visit, Vital, LabResult, ImagingStudy, ConsultReport, HospitalSummary, Immunization
...
def generate_prep_sheet_doc(
    ...
    if past_appointments:
        doc.add_paragraph(f"Last visit: {past_appointments[0].appointment_date.strftime('%m/%d/%Y')}")
        doc.add_heading("Recent Appointments", level=3)
        for appointment in past_appointments:
            status_str = f" ({appointment.status})" if appointment.status else ""
            time_str = appointment.appointment_time.strftime('%I:%M %p') if appointment.appointment_time else ""
            notes_str = f" - {appointment.notes}" if appointment.notes else ""
            doc.add_paragraph(f"• {appointment.appointment_date.strftime('%m/%d/%Y')} {time_str}{status_str}{notes_str}")
    ...
    doc.add_paragraph(f"Date of Birth: {patient.date_of_birth.strftime('%m/%d/%Y')} ({patient.age} years)")
    ...
    if conditions:
        ...
        if active_conditions:
            for condition in active_conditions:
                diagnosed_date = condition.diagnosed_date.strftime('%m/%d/%Y') if condition.diagnosed_date else "Unknown"
    ...
    if vitals and len(vitals) > 0:
        ...
        vitals_data = [
            ...
            ("Height", f"{(latest_vital.height / 2.54):.1f} in" if latest_vital.height else "Not recorded"),
            ("Weight", f"{(latest_vital.weight * 2.20462):.1f} lbs" if latest_vital.weight else "Not recorded"),
            ("BMI", f"{latest_vital.bmi:.1f}" if latest_vital.bmi else "Not recorded"),
            ("Temperature", f"{(latest_vital.temperature * 9/5 + 32):.1f} °F" if latest_vital.temperature else "Not recorded"),
            ("Blood Pressure", f"{latest_vital.blood_pressure_systolic}/{latest_vital.blood_pressure_diastolic} mmHg" if latest_vital.blood_pressure_systolic else "Not recorded"),
            ("Pulse", f"{latest_vital.pulse} bpm" if latest_vital.pulse else "Not recorded"),
            ("Respiratory Rate", f"{latest_vital.respiratory_rate} breaths/min" if latest_vital.respiratory_rate else "Not recorded"),
            ("Oxygen Saturation", f"{latest_vital.oxygen_saturation}%" if latest_vital.oxygen_saturation else "Not recorded")
        ]
    ...
    if conditions:
        ...
        has_diabetes = any("diabetes" in c.name.lower() for c in conditions if c.is_active)
        if has_diabetes:
            standard_screenings.extend(["Diabetic Eye Exam", "Microalbumin", "Foot Exam"])
    ...
    for screen_name in standard_screenings:
        ...
        if screenings:
            matching_screenings = [s for s in screenings if s.screening_type.lower() in screen_name.lower()]
            if matching_screenings:
                ...
                row_cells[2].text = latest_screening.date.strftime('%m/%d/%Y') if latest_screening.date else "N/A"
            else:
    ...
    if labs and len(labs) > 0:
        ...
        for lab in labs[:5]:  # Limit to 5 most recent labs
            ...
            row_cells[1].text = f"{lab.result} {lab.units}" if lab.units else str(lab.result)
            row_cells[2].text = lab.test_date.strftime('%m/%d/%Y') if lab.test_date else "N/A"
    else:
    ...
    if imaging and len(imaging) > 0:
        for study in imaging[:3]:  # Limit to 3 most recent studies
            ...
            if study.findings:
                doc.add_paragraph(f"  Findings: {study.findings}", style='List Bullet 2')
            if study.impression:
                doc.add_paragraph(f"  Impression: {study.impression}", style='List Bullet 2')
    else:
    ...
    if consults and len(consults) > 0:
        for consult in consults[:3]:  # Limit to 3 most recent consults
            ...
            if consult.recommendations:
                doc.add_paragraph(f"  Recommendations: {consult.recommendations}", style='List Bullet 2')
    else:
    ...
    if hospital and len(hospital) > 0:
        for visit in hospital[:3]:  # Limit to 3 most recent hospital visits
            ...
            admission_date = visit.admission_date.strftime('%m/%d/%Y') if visit.admission_date else "Unknown"
            discharge_date = visit.discharge_date.strftime('%m/%d/%Y') if visit.discharge_date else "Unknown"
            p.add_run(f" (Admitted: {admission_date}, Discharged: {discharge_date})")
            if visit.diagnosis:
                doc.add_paragraph(f"  Diagnosis: {visit.diagnosis}", style='List Bullet 2')
            if visit.discharge_summary:
                doc.add_paragraph(f"  Summary: {visit.discharge_summary}", style='List Bullet 2')
    else:
    ...
    if immunizations and len(immunizations) > 0:
        ...
        for immunization in immunizations:
            ...
            row_cells[1].text = immunization.administration_date.strftime('%m/%d/%Y') if immunization.administration_date else "N/A"
