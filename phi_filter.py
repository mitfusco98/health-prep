...
class PHIPatternDetector:
    ...
    def __init__(self, config: PHIFilterConfig, rostered_patient_names: Optional[List[str]] = None):
        self.config = config
        self._compile_patterns()
        self.rostered_patient_names = rostered_patient_names if rostered_patient_names is not None else []
    ...
    def detect_phi_in_text(self, text: str) -> List[Dict]:
        ...
        # Patient names (contextual patterns + rostered names)
        name_detections = []
        if self.config.filter_names:
            # Look for "Patient: Name" patterns
            patient_name_patterns = [
                re.compile(r'(?:Patient|Patient Name|Full Name)\s*[:#]?\s*([A-Za-z]+(?:\s+[A-Zaz]+)*?)(?=\s*\n|$)', re.IGNORECASE),
                re.compile(r'\bName\s*[:#]?\s*([A-Za-z]+(?:\s+[A-Za-z]+)*?)(?=\s*\n|$)', re.IGNORECASE),
                re.compile(r'^Name\s*[:#]?\s*([A-Za-z]+(?:\s+[A-Za-z]+)*?)(?=\s*\n|$)', re.IGNORECASE | re.MULTILINE),
            # Enhanced contextual name patterns
                re.compile(r'^([A-Z][a-z]+\s+[A-Z][a-z]+)(?=\s*\n|\s*$)', re.MULTILINE),  # First line capitalized names only
            ...
            # Check for rostered patient names
            if self.rostered_patient_names:
                for patient_name in self.rostered_patient_names:
                    if len(patient_name) > 3:  # Avoid very short names that might be common words
                        # Create case-insensitive pattern for this specific patient name
        ...
        all_detections = detections + name_detections
        ...
        # Sort by position to enable proper replacement
        return sorted(all_detections, key=lambda x: x['start'])
