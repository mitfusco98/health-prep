...
class KeyValidator:
    ...
    SAFE_PATTERNS = ["anon", "public", "readonly", "limited", "client"]
    ...
    def validate_key_security(
        cls, api_key: str, context: str = "general"
    ) -> Dict[str, any]:
        ...
        if len(api_key) > 500:
            logger.warning(
                f"Unusually long API key ({len(api_key)} chars) - likely service key"
            )
            client_safe = False
        ...
        for key, value in os.environ.items():
            if any(suffix in key.upper() for suffix in ["_KEY", "_SECRET", "_TOKEN"]):
                ...
                if not validation["client_safe"]:
                    logger.info(
                        f"Server-only key detected: {key} ({validation['key_type'].value})"
                    )
                else:
                    logger.info(
                        f"Client-safe key detected: {key} ({validation['key_type'].value})"
                    )
