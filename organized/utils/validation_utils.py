...
def validate_vital_data(form_data: Dict[str, str]) -> List[str]:
    ...
    # Numeric validations
    try:
        if form_data.get("blood_pressure_systolic"):
            systolic = int(form_data["blood_pressure_systolic"])
            if systolic < 50 or systolic > 300:
                errors.append("Systolic blood pressure must be between 50 and 300")
    except ValueError:
        errors.append("Invalid systolic blood pressure value")

    try:
        if form_data.get("blood_pressure_diastolic"):
            diastolic = int(form_data["blood_pressure_diastolic"])
            if diastolic < 30 or diastolic > 200:
                errors.append("Diastolic blood pressure must be between 30 and 200")
    except ValueError:
        errors.append("Invalid diastolic blood pressure value")

    try:
        if form_data.get("heart_rate"):
            heart_rate = int(form_data["heart_rate"])
            if heart_rate < 30 or heart_rate > 300:
                errors.append("Heart rate must be between 30 and 300 BPM")
    except ValueError:
        errors.append("Invalid heart rate value")

    try:
        if form_data.get("temperature"):
            temperature = float(form_data["temperature"])
            if temperature < 90.0 or temperature > 110.0:
    ...
    try:
        if form_data.get("weight"):
            ...
            if weight <= 0 or weight > 1000:
                errors.append("Weight must be between 0 and 1000 lbs")
    except ValueError:
    ...
    try:
        if form_data.get("height"):
            ...
            if height <= 0 or height > 120:
                errors.append("Height must be between 0 and 120 inches")
    except ValueError:
...
def validate_condition_data(form_data: Dict[str, str]) -> List[str]:
    ...
    # Required fields
    if not form_data.get("condition_name", "").strip():
