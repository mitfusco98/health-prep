...
def apply_screening_rules(patient, condition_names):
    ...
    # Lipid Panel
    if age >= 40 or any(
        c in condition_names
        for c in ["diabetes", "hypertension", "heart disease", "hyperlipidemia"]
    ):
        recommendations.append(
            {
                "type": "Lipid Panel",
    ...
    if any(
        c in condition_names
        for c in ["diabetes", "pre-diabetes", "gestational diabetes"]
    ):
        frequency = (
            "Every 3 months" if "diabetes" in condition_names else "Every 6 months"
        )
        recommendations.append(
            {
                "type": "A1C",
        ...
        c in condition_names for c in ["hypertension", "obesity", "pcos"]
    ):
        recommendations.append(
            {
                "type": "A1C",
    ...
    if (sex == "female" and age >= 65) or any(
        c in condition_names for c in ["osteoporosis", "osteopenia"]
    ):
        recommendations.append(
            {
    ...
    if (
        age >= 50
        and age <= 80
        and any(c in condition_names for c in ["smoking", "tobacco use"])
    ):
        recommendations.append(
            {
