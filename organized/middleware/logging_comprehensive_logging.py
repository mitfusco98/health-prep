    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                standardized_operation = standardized_ops.get(
                    operation_type.lower(), operation_type
                )
                        ...
                        if key.lower() not in sensitive_fields:
                            ...
                            # Medical data specific fields with enhanced details
                                ...
                                if key in ["hospital_name", "facility"]:
                                    ...
                                    log_details["admitting_diagnosis"] = str(value)[
                                        :200
                                    ]
                ...
                try:
                    error_details = {
                        ...
                        "patient_name": (
                            patient_name if "patient_name" in locals() else "Unknown"
                        ),
                        "function_name": f.__name__,
                        "error_message": str(e),
                        "user_id": session.get("user_id"),
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                if request.method in ["POST", "PUT", "PATCH"] and request.form:
                    ...
                    if data_type == "alert":
                        log_details["action"] = (
                            "add" if request.method == "POST" else "edit"
                        )
                        ...
                        log_details["alert_notes"] = alert_details[
                            :500
                        ]  # Explicit notes field
                        ...
                        log_details["priority"] = (
                            alert_severity  # Map severity to priority for consistency
                        )
                    elif data_type == "condition":
                        log_details["action"] = (
                            "add" if request.method == "POST" else "edit"
                        )
                        log_details["condition_name"] = request.form.get(
                            "name",
                            request.form.get(
                                "condition_name", request.form.get("diagnosis", "")
                            ),
                        )
                        log_details["diagnosis_date"] = request.form.get(
                            "diagnosed_date",
                            request.form.get("diagnosis_date", current_date),
                        )
                        log_details["diagnosis_time"] = current_time
                        log_details["severity"] = request.form.get("severity", "")
                        log_details["status"] = request.form.get(
                            "status",
                            "Active" if request.form.get("is_active") else "Inactive",
                        )
                        ...
                        log_details["vital_date"] = request.form.get(
                            "date",
                            request.form.get(
                                "vital_date",
                                request.form.get("measurement_date", current_date),
                            ),
                        )
                        log_details["vital_time"] = current_time
                        log_details["blood_pressure"] = request.form.get(
                            "blood_pressure", ""
                        )
                        if (
                            not log_details["blood_pressure"]
                            and request.form.get("blood_pressure_systolic")
                            and request.form.get("blood_pressure_diastolic")
                        ):
                            log_details["blood_pressure"] = (
                                f"{request.form.get('blood_pressure_systolic')}/{request.form.get('blood_pressure_diastolic')}"
                            )
                        log_details["heart_rate"] = request.form.get(
                            "heart_rate", request.form.get("pulse", "")
                        )
                        ...
                        log_details["oxygen_saturation"] = request.form.get(
                            "oxygen_saturation", ""
                        )
                        log_details["respiratory_rate"] = request.form.get(
                            "respiratory_rate", ""
                        )
                        ...
                        log_details["test_name"] = request.form.get(
                            "test_name", request.form.get("lab_name", "")
                        )
                        log_details["test_date"] = request.form.get(
                            "test_date", request.form.get("lab_date", current_date)
                        )
                        log_details["test_time"] = current_time
                        log_details["result_value"] = request.form.get(
                            "result_value",
                            request.form.get("result", request.form.get("value", "")),
                        )
                        log_details["unit"] = request.form.get("unit", "")
                        log_details["reference_range"] = request.form.get(
                            "reference_range", ""
                        )
                        log_details["is_abnormal"] = bool(
                            request.form.get("is_abnormal")
                        )
                    elif data_type == "immunization":
                        log_details["action"] = (
                            "add" if request.method == "POST" else "edit"
                        )
                        log_details["vaccine_name"] = request.form.get(
                            "vaccine_name", request.form.get("immunization_name", "")
                        )
                        log_details["vaccination_date"] = request.form.get(
                            "administration_date",
                            request.form.get(
                                "vaccination_date",
                                request.form.get("immunization_date", current_date),
                            ),
                        )
                        log_details["vaccination_time"] = current_time
                        log_details["lot_number"] = request.form.get("lot_number", "")
                        log_details["dose_number"] = request.form.get("dose_number", "")
                        log_details["manufacturer"] = request.form.get(
                            "manufacturer", ""
                        )
                        ...
                        log_details["imaging_type"] = request.form.get(
                            "study_type", request.form.get("imaging_type", "")
                        )
                        log_details["imaging_date"] = request.form.get(
                            "study_date", request.form.get("imaging_date", current_date)
                        )
                        log_details["imaging_time"] = current_time
                        log_details["body_site"] = request.form.get(
                            "body_site", request.form.get("location", "")
                        )
                        log_details["findings"] = request.form.get("findings", "")[:200]
                        log_details["impression"] = request.form.get("impression", "")[
                            :200
                        ]
                        ...
                        log_details["specialist"] = request.form.get(
                            "specialist", request.form.get("consultant", "")
                        )
                        log_details["specialty"] = request.form.get(
                            "specialty", request.form.get("speciality", "")
                        )
                        log_details["consult_date"] = request.form.get(
                            "report_date",
                            request.form.get("consult_date", current_date),
                        )
                        log_details["consult_time"] = current_time
                        log_details["referral_reason"] = request.form.get(
                            "reason", request.form.get("referral_reason", "")
                        )[:200]
                        log_details["findings"] = request.form.get("findings", "")[:200]
                        log_details["recommendations"] = request.form.get(
                            "recommendations", ""
                        )[:200]
                        ...
                        log_details["hospital_name"] = request.form.get(
                            "hospital_name", request.form.get("facility", "")
                        )
                        log_details["admission_date"] = request.form.get(
                            "admission_date", current_date
                        )
                        log_details["admission_time"] = current_time
                        log_details["discharge_date"] = request.form.get(
                            "discharge_date", ""
                        )
                        log_details["admitting_diagnosis"] = request.form.get(
                            "admitting_diagnosis", ""
                        )[:200]
                        log_details["discharge_diagnosis"] = request.form.get(
                            "discharge_diagnosis", ""
                        )[:200]
                        log_details["procedures"] = request.form.get("procedures", "")[
                            :200
                        ]
                    ...
                    if data_type == "alert" and record_id:
                        try:
                            ...
                            if alert_to_delete:
                                ...
                                log_details["deleted_alert_description"] = (
                                    alert_to_delete.description or ""
                                )
                                log_details["deleted_alert_details"] = (
                                    alert_to_delete.details or ""
                                )
                                log_details["deleted_alert_notes"] = (
                                    alert_to_delete.details or ""
                                )  # Explicit notes field
                                log_details["deleted_alert_type"] = (
                                    alert_to_delete.alert_type or ""
                                )
                                log_details["deleted_severity"] = (
                                    alert_to_delete.severity or ""
                                )
                                log_details["deleted_priority"] = (
                                    alert_to_delete.severity or ""
                                )
                                log_details["deleted_start_date"] = (
                                    alert_to_delete.start_date.strftime("%Y-%m-%d")
                                    if alert_to_delete.start_date
                                    else ""
                                )
                                log_details["deleted_end_date"] = (
                                    alert_to_delete.end_date.strftime("%Y-%m-%d")
                                    if alert_to_delete.end_date
                                    else ""
                                )
                                log_details["deleted_is_active"] = (
                                    alert_to_delete.is_active
                                )
                                log_details["deletion_date"] = current_date
                                log_details["deletion_time"] = current_time
                        except Exception as alert_error:
                            logger.warning(
                                f"Could not capture alert details for deletion: {str(alert_error)}"
                            )
                    ...
                    if request.files:
                        for file_key, file_obj in request.files.items():
                            if file_obj and file_obj.filename:
                                ...
                                log_details["file_type"] = (
                                    file_obj.content_type or "unknown"
                                )
                        ...
                        log_details["file_name"] = log_details.get(
                ...
                if data_type == "alert":
                    event_type_name = (
                        f'alert_{log_details.get("action", "modification")}'
                    )
