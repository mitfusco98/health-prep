...
def manage_screening_types():
    ...
    return render_template(
        "admin/screening_types.html", screening_types=screening_types
    )
...
def edit_screening_type(type_id):
    ...
    if request.method == "POST":
        try:
            ...
            screening_type.frequency_months = int(
                request.form.get("frequency_months", 12)
            )
    ...
    return render_template(
        "admin/edit_screening_type.html", screening_type=screening_type
    )
...
def system_health():
    ...
    try:
        ...
        error_logs = (
            AdminLog.query.filter(AdminLog.event_type.like("%error%"))
    ...
    try:
        stats = {
            ...
            "active_users": (
                ...
                else User.query.count()
            ),
            "recent_activity": AdminLog.query.order_by(desc(AdminLog.timestamp))
            .limit(5)
            .count(),
        }
