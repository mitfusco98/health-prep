...
def patient_documents(patient_id):
    ...
    documents = MedicalDocument.query.filter_by(patient_id=patient_id).order_by(MedicalDocument.upload_date.desc()).all()
    
    return render_template('documents/patient_documents.html', patient=patient, documents=documents)

@document_bp.route('/patient/<int:patient_id>/documents/upload', methods=['GET', 'POST'])
        ...
        if file and allowed_file(file.filename):
            try:
                ...
                document = MedicalDocument(
                    ...
                    date_created=datetime.strptime(document_data['date_created'], '%Y-%m-%d').date() if document_data['date_created'] else datetime.now().date(),
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if new_data['date_created']:
                document.date_created = datetime.strptime(new_data['date_created'], '%Y-%m-%d').date()
