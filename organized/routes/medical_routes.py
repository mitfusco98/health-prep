...
from organized.utils.validation_utils import validate_vital_data, validate_condition_data
...
def patient_conditions(patient_id):
        ...
        if not errors:
            try:
                condition = Condition(
                    ...
                    diagnosis_date=datetime.strptime(condition_data['diagnosis_date'], '%Y-%m-%d').date() if condition_data['diagnosis_date'] else None,
    ...
    return render_template('medical/conditions.html', patient=patient, conditions=conditions)
...
def patient_immunizations(patient_id):
    ...
    if request.method == 'POST':
        ...
        try:
            immunization = Immunization(
                ...
                date_administered=datetime.strptime(immunization_data['date_administered'], '%Y-%m-%d').date(),
    ...
    return render_template('medical/immunizations.html', patient=patient, immunizations=immunizations)
