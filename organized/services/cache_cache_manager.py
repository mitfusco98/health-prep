...
class CacheManager:
    ...
    def set(self, key, value, timeout=300):
        ...
        if len(self.memory_cache) > 1000:
            ...
            expired_keys = [
                k
                for k, ts in self.memory_cache_timestamps.items()
...
def cache_route(timeout=300, vary_on=None):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            # Generate cache key
            cache_key_parts = [func.__name__, request.method, request.path]
...
def invalidate_patient_cache(patient_id=None):
    ...
    patterns = ["api_patients*", "patients*", "home_dashboard*"]

    if patient_id:
        patterns.extend(
            [f"patient_detail_{patient_id}*", f"api_patients_{patient_id}*"]
        )
...
def invalidate_api_cache():
    ...
    patterns = ["api_patients*", "api_appointments*"]
