...
from models import Patient, MedicalDocument, Condition, Vital, Immunization, PatientAlert, Screening, ScreeningType
from app import db
import logging
...
class PatientService:
    ...
    @staticmethod
    def update_patient(patient_id: int, patient_data: Dict) -> Tuple[Optional[Patient], List[str]]:
        ...
        try:
            ...
            patient.emergency_contact = patient_data.get('emergency_contact', patient.emergency_contact)
            patient.emergency_phone = patient_data.get('emergency_phone', patient.emergency_phone)
            patient.insurance_provider = patient_data.get('insurance_provider', patient.insurance_provider)
            patient.insurance_number = patient_data.get('insurance_number', patient.insurance_number)
            patient.primary_physician = patient_data.get('primary_physician', patient.primary_physician)
    ...
    def get_patient_statistics() -> Dict:
        ...
        try:
            ...
            # Gender distribution
            gender_stats = db.session.query(Patient.gender, db.func.count(Patient.id)).group_by(Patient.gender).all()
            ...
            return {
                ...
                'patients_with_alerts': PatientAlert.query.filter_by(is_active=True).count(),
                'patients_with_due_screenings': len(set([s.patient_id for s in Screening.query.filter(Screening.due_date <= today).all()]))
            }
    ...
    def bulk_delete_patients(patient_ids: List[int]) -> Tuple[int, List[str]]:
        ...
        for patient_id in patient_ids:
            try:
                success, patient_errors = PatientService.delete_patient_with_records(patient_id)
                if success:
                    deleted_count += 1
                else:
