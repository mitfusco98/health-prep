...
def validate_date_input(
    date_str, field_name="date", required=False, date_format="%Y-%m-%d"
):
    """
...
def validate_string_field(
    value, field_name, min_length=0, max_length=None, pattern=None, allow_empty=False
):
    """
    ...
    try:
        ...
        if isinstance(validation_errors, list) and validation_errors:
            ...
            if isinstance(validation_errors[0], dict) and "loc" in validation_errors[0]:
                ...
                error_details = {
                    f"error_{i}": error for i, error in enumerate(validation_errors)
                }
        else:
        ...
        AdminLog.log_event(
            ...
            event_details={
                ...
                "error_count": (
                    len(validation_errors) if isinstance(validation_errors, list) else 1
                ),
            },
        ...
        logger.warning(
            f"Validation error logged: {context} - {len(validation_errors) if isinstance(validation_errors, list) else 1} errors"
        )
            ...
            if request.method == "POST":
                ...
                if errors:
                    ...
                    log_validation_error_unified(
                        f"{route_func.__name__}_validation", errors, form_data, user_id
                    )
