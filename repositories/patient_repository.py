...
class PatientRepository:
    ...
    def get_patients_with_conditions(condition_names):
        ...
        return Patient.query.join(Condition).filter(
            Condition.name.in_(condition_names),
    ...
    def get_next_available_mrn():
        ...
        try:
            result = db.session.execute(
                text("SELECT MAX(CAST(SUBSTRING(mrn FROM '[0-9]+') AS INTEGER)) FROM patient WHERE mrn ~ '^[0-9]+$'")
            )
