...
class AppointmentSchema(BaseModel):
    ...
    status: Optional[str] = Field("Scheduled", regex="^(Scheduled|Completed|Cancelled|No Show)$")
...
class DocumentSchema(BaseModel):
    ...
    def validate_document_type(cls, v):
        allowed_types = ['Lab Report', 'Imaging', 'Consultation', 'Prescription', 'Insurance', 'Other']
...
class ConditionSchema(BaseModel):
    ...
    patient_id: int = Field(..., gt=0, description="Patient ID")
    condition_name: str = Field(..., min_length=1, max_length=200, description="Condition name")
...
class VitalSchema(BaseModel):
    ...
    blood_pressure_systolic: Optional[int] = Field(None, ge=50, le=300, description="Systolic BP")
    blood_pressure_diastolic: Optional[int] = Field(None, ge=30, le=200, description="Diastolic BP")
    ...
    height: Optional[float] = Field(None, ge=10.0, le=120.0, description="Height (inches)")
...
class ImmunizationSchema(BaseModel):
    ...
    patient_id: int = Field(..., gt=0, description="Patient ID")
    vaccine_name: str = Field(..., min_length=1, max_length=100, description="Vaccine name")
    administration_date: date = Field(..., description="Administration date")
    lot_number: Optional[str] = Field(None, max_length=50, description="Vaccine lot number")
    administered_by: Optional[str] = Field(None, max_length=100, description="Administered by")
...
class AlertSchema(BaseModel):
    ...
    patient_id: int = Field(..., gt=0, description="Patient ID")
    alert_type: str = Field(..., regex="^(Allergy|Medical Alert|Insurance|Contact|Other)$")
...
class ScreeningTypeSchema(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=200, description="Screening type name")
    description: Optional[str] = Field(None, max_length=500, description="Screening description")
    frequency_months: Optional[int] = Field(None, ge=1, le=120, description="Frequency in months")
    ...
    def validate_age_range(cls, v, values):
        if v and 'age_start' in values and values['age_start'] and v <= values['age_start']:
...
class BulkDeleteSchema(BaseModel):
    ...
    appointment_ids: Optional[str] = Field(None, description="Comma-separated appointment IDs")
    selected_appointments: Optional[List[str]] = Field(None, description="List of appointment IDs")
    selected_patients: Optional[List[str]] = Field(None, description="List of patient IDs")
